...
${order.mobile || ""}`;
  ...
  const buildGstInvoiceHtml = (order) => {
    ...
    const invoiceLogoUrl = `${window.location.origin}/banners/logo-banner.png`;
    const authorizedSignatureUrl = `${window.location.origin}/authorized-signature.png`;
    ...
    return `<!doctype html><html><head><meta charset="utf-8"><title>Tax Invoice - ${escapeHtml(order.orderId)}</title>
      <style>
        *{box-sizing:border-box}body{margin:0;background:#eee;color:#17211b;font:12px Arial,sans-serif}.page{width:297mm;min-height:210mm;margin:auto;padding:12mm;background:#fff}.top{display:flex;justify-content:space-between;gap:24px;border-bottom:2px solid #164c2b;padding-bottom:10px}.seller{flex:1;min-width:0}.invoiceLogo{display:block;width:330px;max-width:100%;height:76px;object-fit:contain;object-position:left center;margin:0 0 7px;border-radius:4px}.top h2{margin:0;text-align:right}.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px}.box{border:1px solid #bbb;padding:9px}.box h3{margin:0 0 6px;color:#164c2b}p{margin:2px 0;line-height:1.45}table{width:100%;border-collapse:collapse;margin-top:10px}th,td{border:1px solid #bbb;padding:6px;vertical-align:top}th{background:#eaf6ee}.num{text-align:right;white-space:nowrap}.totals{display:grid;grid-template-columns:1fr 320px;gap:12px;margin-top:10px}.totalLine{display:flex;justify-content:space-between;border-bottom:1px solid #ddd;padding:5px}.grand{font-size:15px}.footer{display:flex;justify-content:space-between;margin-top:18px;align-items:flex-end}.sign{text-align:center;min-width:220px}.signatureMark{position:relative;width:210px;height:60px;overflow:hidden;margin:0 auto 3px;border-bottom:1px solid #222}.signatureMark img{position:absolute;width:310px;height:auto;left:50%;top:50%;transform:translate(-50%,-50%)}.actions{position:sticky;top:0;background:#222;padding:8px;text-align:center}.actions button{padding:8px 14px;background:#159451;color:white;border:0;border-radius:5px}@media print{body{background:#fff}.actions{display:none}.page{margin:0;padding:0;width:auto}.invoiceLogo,.signatureMark img{print-color-adjust:exact;-webkit-print-color-adjust:exact}}
      </style></head><body><div class="actions"><button onclick="window.print()">Print / Save PDF</button></div><main class="page">
      <section class="top"><div class="seller"><img class="invoiceLogo" src="${escapeHtml(invoiceLogoUrl)}" alt="${escapeHtml(businessConfig.tradeName)}"><p><b>Legal Name:</b> ${escapeHtml(businessConfig.legalName)}</p><p><b>GSTIN:</b> ${escapeHtml(businessConfig.gstin)}</p><p>${sellerAddress}</p><p><b>State:</b> ${escapeHtml(businessConfig.stateName)} | <b>State Code:</b> ${escapeHtml(businessConfig.stateCode)}</p></div><div><h2>TAX INVOICE</h2><p><b>Invoice No:</b> ${escapeHtml(order.invoiceNumber || buildBillNumber(order))}</p><p><b>Invoice Date:</b> ${escapeHtml(formatDate(order.invoiceDate || order.createdAt))}</p><p><b>Order No:</b> ${escapeHtml(order.orderId)}</p><p><b>Payment:</b> ${escapeHtml(payment.status)} | ${escapeHtml(payment.type)}</p></div></section>
      ...
      <section class="footer"><p>This is a computer-generated tax invoice.</p><div class="sign"><div class="signatureMark"><img src="${escapeHtml(authorizedSignatureUrl)}" alt="Authorized signature"></div>Authorized Signatory<br>${escapeHtml(businessConfig.tradeName)}</div></section></main></body></html>`;
  };
